...
public class TestPow {
    ...
    public static void runPowerTest(String catalogName, int amountOfTests, BiFunction<Double, Integer, Double> biFunction) {
        runPowerTest(catalogName, amountOfTests, 5000, biFunction);
    }
    public static void runPowerTest(String catalogName, int amountOfTests, int timeout, BiFunction<Double, Integer, Double> biFunction) {
        for (int i = 0; i <= amountOfTests; i++) {
            ...
            try {
                myAnswer = CompletableFuture
                        .supplyAsync(() -> biFunction.apply(Double.parseDouble(n[0]), Integer.parseInt(n[1])))
